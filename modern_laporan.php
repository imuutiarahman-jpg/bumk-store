...
function hitungVisible(){let ts=0,tn=0,vis=0;const rows=document.querySelectorAll('.brg');rows.forEach(tr=>{if(tr.style.display!=='none'){ts+=+(tr.dataset.stok||0);tn+=+(tr.dataset.nilai||0);vis++;}});const gu=document.getElementById('grandUnit'),gv=document.getElementById('grandVal');if(gu)gu.textContent=ts.toLocaleString('id-ID');if(gv)gv.textContent=rupiahJS(tn);const fl=document.querySelector('tfoot .foot-label');if(fl)fl.title=vis+' dari '+rows.length+' produk tampil (mengikuti filter/pencarian)';}
